...
export const getSelectedFilters = (filtersState: FiltersState, filter: LocationFilterVariant) => {
  ...
  return isLocationFilterSelected(filter, filtersState) && selectedFilters.length > 0 ? selectedFilters : undefined
}

export const getSelectedServiceClasses = (filtersState: FiltersState) => {
  const selectedFilters = filtersState.serviceClassFilters
  return selectedFilters.length > 0 ? selectedFilters.map((item) => item.uri) : undefined
}

export const getSelectedTargetGroups = (filtersState: FiltersState) => {
  const selectedFilters = filtersState.targetGroupFilters
  return selectedFilters.length > 0 ? selectedFilters : undefined
}

export const getSelectedFundingType = (filtersState: FiltersState) => {
  const selectedFilters = filtersState.fundingType
  return selectedFilters.length > 0 && selectedFilters.length !== fundingTypeFilters.length ? selectedFilters : undefined
}
...
export const getVisibleFilters = (featureFlags: string[]) => {
  const initialFilters = [municipalitiesFilter, regionFilter, hospitalDistrictFilter, wellbeingCountyFilter]

  return featureFlags.includes('hide_hospital_districts') ? initialFilters.filter((f) => f.type !== 'hospitalDistrict') : initialFilters
}
